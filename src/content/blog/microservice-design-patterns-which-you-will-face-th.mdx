...
ตัวอย่าง: ระบบ Shopping Cart — แทนที่จะเก็บแค่ item list ปัจจุบันของ cart เราเก็บทุก action ที่ user ทำ: "added item A", "changed quantity of item A to 3", "removed item B", "applied coupon X" ทั้งหมดนี้เรียงตามเวลา
...
ตัวอย่าง: ระบบ Order ของ food delivery app — เมื่อ user สั่งอาหาร Order Service สร้าง order แล้ว publish event, Payment Service รับ event ไปตัดเงิน, Restaurant Service รับ event ไป confirm กับร้าน ถ้า payment fail ก็ publish event กลับมาให้ Order Service cancel order
...
sequenceDiagram
    ...
    OC->>OS: Update Order → Confirmed
    Note over OC: If any step fails,\norchestrator triggers\ncompensating transactions
`} />
...
flowchart TB
    ...
    PRO -->|runs contract\nas test suite| TEST[Provider Tests]
    TEST -->|pass/fail| RESULT[Integration\nVerified]
`} />
...
sequenceDiagram
    ...
    Note over Cache,DB: Cache and DB\nalways in sync
`} />
