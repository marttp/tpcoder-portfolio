...
กระบวนการนี้ไม่ได้ให้เอกสาร plan ออกมา สิ่งที่ได้คือหัวที่โล่ง ควรอธิบายได้ว่ากำลังสร้างอะไร ลำดับไหน เพราะอะไร ตั้งแต่ก่อนเขียน function แรก

อีกเรื่องหนึ่ง: ticket คือรูปแบบที่มีโครงสร้างมากที่สุดที่มี เมื่อโตขึ้น input จะมีโครงสร้างน้อยลง ไม่ใช่มากขึ้น อาจเป็น product requirements document ที่ต้องดึง task ออกมาเอง อาจเป็น direction แบบกว้างๆ จาก tech lead ที่ต้องแปลงเป็น ticket เอง หรืออาจเป็นแค่ business goal ที่ไม่มี spec และต้องออกแบบทุกอย่างจากศูนย์
...
ขอเคลียร์ก่อน: เรื่องที่จะพูดไม่เกี่ยวกับการ grind LeetCode ประเด็นคือการเข้าใจ data structures ดีพอจะรู้ว่าเมื่อไหร่มีการเลือกใช้ผิดอัน
...
**Linked lists** — ไม่ค่อยใช้โดยตรง แต่จำเป็นสำหรับการเข้าใจ queues, LRU caches, และการทำงานของ memory allocation ข้างใน อีกทั้งการคิดแบบ pointer ยังช่วยเรื่อง tree traversal

**Trees** — BST, heap, trie Heap สำคัญสำหรับ priority queues Trie สำคัญสำหรับ prefix search BST สำคัญสำหรับ sorted data operations ไม่ต้อง implement เองได้จากหัว แต่ต้องรู้ว่าเมื่อไหร่ควร reach for แต่ละอัน
...
production incidents ส่วนใหญ่ที่เจอมามีต้นเหตุจาก database problem — ไม่ว่าจะเป็น missing index, bad query, transaction deadlock, หรือ consistency model ที่เข้าใจผิด Databases คือที่ที่ fundamentals สำคัญที่สุดสำหรับงาน backend ประจำวัน
...
นี่คือ performance issue ที่เจอบ่อยที่สุด และมาจากการไม่เข้าใจว่า queries ประกอบกันยังไง
...
`EXPLAIN` (หรือ `EXPLAIN ANALYZE` ใน PostgreSQL) แสดงให้เห็นว่า database จะ execute query ยังไง ต้องรู้วิธีอ่าน สัญญาณสำคัญ:
